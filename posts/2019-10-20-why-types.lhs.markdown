...
Stepping back from these examples, I'd like to reiterate that 
the point is *not* that types and program logics are at odds 
with each other. Instead, the lesson is that classical 
Floyd-Hoare logic, associates invariants with *program* 
positions, and that Liquid/Refinement types are a 
*generalization* that _also_ let you associate 
invariants with *type* positions. This generalization 
lets us exploit and profit from the fact that 

* types are a kind of program logic, and
* syntax-directed typing rules are a kind of decision procedure,

that, in many common situations, allow decomposing verification 
into simple, quantifier-free, SMT-friendly formulas. 

As you might imagine, the benefits are even more pronounced when
working with higher-order functions, e.g. `map`-ing or `fold`-ing 
over containers...
