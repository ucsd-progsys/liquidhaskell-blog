...
However, `tesj1` already establishes that `S.member 2 (rng 1 3)`
...
Equational proofs like `tesj1` and `tesj2` often
...
just the important bits: the recursive (inductive) skeleton:
...
Notice that the above is exactly the original proof
sans the intermediate equalities, which LH can
